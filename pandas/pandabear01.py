...
# CHALLENGE 03 (OPTIONAL)- Export your final dataframe to different types. You can do this using the .to_* methods. For example, dataframe.to_excel('myexcel.xls') will create an excel file called myexcel.xls. Just a few of the other methods include to_json() and to_csv(). You can use print(dir(movies)) to see the methods avail to your dataframe. You may need to perform some python3 -m pip installs to be successful exporting data.

# CHALLENGE 04 (OPTIONAL)- Before exporting your data, manipulate it to a different format (order by title, year rel, budget) or maybe eliminate some columns.
